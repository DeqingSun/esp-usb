...
#define CDC_ACM_CTRL_TIMEOUT_MS    (1000) // Every CDC device should be able to respond to CTRL transfer in 5 seconds
...
esp_err_t cdc_acm_host_enable_remote_wakeup(cdc_acm_dev_hdl_t cdc_hdl, bool enable)
{
    ...
    const usb_config_desc_t *config_desc;
    ESP_ERROR_CHECK(usb_host_get_active_config_descriptor(this_cdc_dev->dev_hdl, &config_desc));
    ...
    SLIST_FOREACH(cdc_dev, &p_cdc_acm_obj->cdc_devices_list, list_entry) {
        if (cdc_dev->dev_hdl == this_cdc_dev->dev_hdl) {
            if (cdc_dev->remote_wakeup_enabled) {
    ...
    if (remote_wake_enabled == enable) {
        ESP_LOGW(TAG, "Remote wakeup already %s on this device", (enable) ? "enabled" : "disabled");
        return ESP_OK;
    }
    ...
    ESP_GOTO_ON_FALSE(this_cdc_dev->ctrl_transfer->status == USB_TRANSFER_STATUS_COMPLETED, ESP_ERR_INVALID_RESPONSE, unblock, TAG, "Control transfer error");
    ESP_GOTO_ON_FALSE(this_cdc_dev->ctrl_transfer->actual_num_bytes == this_cdc_dev->ctrl_transfer->num_bytes, ESP_ERR_INVALID_RESPONSE, unblock, TAG, "Incorrect number of bytes transferred");
    ret = ESP_OK;
